...
- 当前仍在 `dev` 分支开发，`main` 只用于正式版本发布。
- 用户提供了新的 UI 设计稿和图标/资源参考，要求结合 `docs/ui-mockup/` 设计文档重做 App UI。
- 资源读取边界：只读取项目目录、用户明确指定的 `C:/Users/yigeh/Desktop/用户附件.png`，以及聊天框内提供的设计图；不要枚举或打开桌面上的其他文件。
- 本轮 UI 目标：保留现有业务逻辑和页面拆分，重点重做主题配色、首页信息层级、设置页分区控件、关于页入口列表。
- 设计方向：浅色为暖奶油底色，深色为森林绿黑底色；主色使用绿色，金币/薪资提示使用金色；卡片、按钮和芯片统一 12dp 左右圆角。
- 首页要保持一屏可读：今日已赚、金额、法定/固定工作日来源、进度、已进行/剩余、每秒/每小时收入、当前状态、摘要和设置按钮。
- 设置页继续保持“更改立即生效”，不恢复保存按钮；长内容允许滚动。
- 关于页展示 App 图标、名称、版本号、描述、隐私说明、GitHub 和反馈入口。

## 2026-05-27 UI B 方案继续贴近设计稿

- 用户认为上一版 UI 与设计图差距仍较大，确认按 B 方案继续：先做到约 80% 设计还原，而不是只微调现有 UI 或做像素级重做。
- B 方案范围：保留现有业务逻辑和页面路由，重点补足手绘风视觉组件、设计稿式分段控件、首页/设置/关于的页面密度和插画氛围。
